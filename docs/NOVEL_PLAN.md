...
为媒体资源爬虫管理系统新增**小说爬取与阅读**功能模块，与**动漫资源模块**平级独立。支持从小说镜像站点（如八一中文网）抓取小说元数据、章节列表和正文，提供在线阅读体验。

**架构说明：**
- 🎬 **动漫资源** — 独立模块，包含动漫画廊、分区浏览、在线播放器、RSS 订阅等
- 📚 **小说资源** — 独立模块，包含小说书库、章节目录、在线阅读器等
- 🏠 **统一首页** — `/` 显示两个模块的入口卡片，用户选择进入对应功能

两个模块互不干扰，各有独立的导航和数据表。
...
#### 统一首页（/）
- 显示"动漫资源"和"小说资源"两个入口卡片
- 快捷链接：数据库后台、API 调试、RSS 订阅
...
### ✅ v4.0.1 - 架构优化（2026-06-04）

- [x] 动漫与小说平级化（两个独立功能模块）
- [x] 新增统一首页（显示动漫与小说入口卡片）
- [x] 导航结构优化（各模块独立导航，移除交叉引用）
- [x] 文档同步更新（README/NOVEL_PLAN）
