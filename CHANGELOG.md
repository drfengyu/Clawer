...
### 🔧 架构调整
- ✅ **动漫与小说平级化** — 动漫与小说改为平级的两个独立功能模块，互不干扰
- ✅ **新增统一首页** `/` — 显示"动漫资源"和"小说资源"两个入口卡片，替代原来直接跳转到画廊
- ✅ **导航结构优化** — 动漫页面（画廊/时间轴）导航栏移除小说入口，改为返回首页；小说页面独立导航
- ✅ **文档同步更新** — README 更新为模块化结构说明，明确动漫与小说为两个独立功能

### 📝 文档改进
- ✅ **README 重构** — 功能特性分为"动漫资源模块"、"小说资源模块"、"通用功能"三大板块
- ✅ **快速开始更新** — 说明首页导航结构与各模块使用方法
...
#### 小说爬取与阅读（NEW）
- ✅ **小说爬虫引擎** — 支持八一中文网（81zw2.com），元数据 + 章节列表 + 正文按需抓取
- ✅ **小说书库** `/novels` — 网格展示，搜索（书名/作者），分页（每页 60 本）
- ✅ **小说详情** `/novel/:id` — 封面 + 书籍信息（作者/分类/状态/字数/简介）+ 章节目录
- ✅ **在线阅读器** `/novel/:novelId/read/:chapterId` — 宋体大行距排版，上下章导航，键盘快捷键（← →）
- ✅ **按需抓取** — 章节正文仅在用户阅读时实时爬取（避免批量抓取压力）
- ✅ **数据持久化** — SQLite 存储小说元数据（novels 表）与章节内容（novel_chapters 表）
- ✅ **导航集成** — 画廊 + 时间轴页面新增"📚 小说"入口
- ⚠️ **版权声明** — 仅供个人学习研究，详见 [docs/NOVEL_PLAN.md](./docs/NOVEL_PLAN.md)

### 📝 文档改进
- ✅ **小说功能开发计划** — 新增 `docs/NOVEL_PLAN.md`，包含功能目标、技术方案、版权声明、开发路线图
- ✅ **README 更新** — 功能特性新增"小说功能"章节
...
- 🔧 **修正增量同步描述** — 更正为"从第 1 页往后爬，只入库新片，连续 3 页无新增即提前结束"（非固定前 3 页）
