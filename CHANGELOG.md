...
### ✨ 新增功能
- ✅ **每日更新时间轴** `/timeline` — 按天分组回顾历史每日更新，每条显示标题 + 该集简介（正文简介缺省回退到"更新至X集"状态），点击跳转详情页；含周几、当天更新数、按天分页（每页 14 天）
- ✅ 画廊顶栏新增「时间轴」入口
...
### ✨ 新增功能
- ✅ **RSS 订阅源** — 新增只读端点 `GET /rss`（每日更新）与 `GET /rss/category/:name`（分区），输出标准 RSS 2.0，可用任意阅读器/服务（Inoreader、Follow、RSSHub 等）订阅；画廊顶栏加「📡 RSS」入口
- ✅ **每日主动推送** — 调度器每日爬完后，把"今日新增"动漫推送到 **Bark**（iOS）/ **Server酱**（微信）/ **邮件(SMTP)**；各渠道按 `.env` 开关，未配置静默跳过，单渠道失败不影响其它
- ✅ **手动联调端点** `POST /api/push/test` — 不用等到 8 点即可验证推送配置，返回各渠道结果
- ✅ 新增依赖 `nodemailer`（邮件推送）；新增文档 [docs/RSS_PUSH.md](./docs/RSS_PUSH.md)
