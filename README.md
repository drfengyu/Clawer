...
一个基于 Node.js 的爬虫系统，支持智能识别网站类型，抓取并管理音视频图片资源。包含 **动漫资源模块**（B站风格画廊 + 在线播放器）与 **小说资源模块**（书库 + 在线阅读器）。
...
**首页导航：**
- 🎬 **动漫资源** — 进入动漫画廊（每日更新/分区浏览/在线播放）
- 📚 **小说资源** — 进入小说书库（书库列表/在线阅读）
...
**小说资源抓取：**
- 访问 `/novels` 书库页面，点击「爬取首页」获取小说列表
- 点击小说卡片进入详情页，点击「爬取章节列表」获取目录
- 章节正文按需抓取（点击阅读时自动抓取）

> 快速填充库：新环境启动后立即运行 `npm run sync:categories` 全量同步动漫资源，然后等每天 8 点自动更新即可。
